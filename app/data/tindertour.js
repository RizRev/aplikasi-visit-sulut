...
export async function getTinderTour() {
  try {
    let response = await fetch('http://b.visit-northsulawesi.com/api/client/list');
    let data = await response.json();
    
    console.log("DATA SWIPE : ",data);
    if (data.success && data.result && data.result.length > 0) {
      // Extracting the necessary data from the API result
      const tinderTourData = data.result.map((item) => {
        return {
          id: item.id,
          expoPushToken: item.expoPushToken,
          name: item.name,
          language: item.language,
          imageSrc: item.imageProfile[0]?.url || null, // Using optional chaining to handle missing imageSrc
        };
      });

      return tinderTourData;
    } else {
...
export const TinderTourPics = shuffleArray([
  {
