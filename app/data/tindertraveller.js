...
export async function getTinderTraveller() {
  try {
    let response = await fetch('http://b.visit-northsulawesi.com/api/user/list');
    let data = await response.json();
    
    // console.log("DATA SWIPE : ",data);
    if (data.success && data.result && data.result.length > 0) {
      // Extracting the necessary data from the API result
      const tinderTravellerData = data.result.map((item) => {
        return {
          id: item.id,
          expoPushToken: item.expoPushToken,
          name: item.name ,
          imageSrc: item.image || null, // Using optional chaining to handle missing imageSrc
        };
      });

      return tinderTravellerData;
    } else {
