import {Images} from '@config';

export async function getHotels() {
  try {
    let response = await fetch('http://b.visit-northsulawesi.com/api/hotel/list');
    // let response = await fetch('http://be.visit-northsulawesi.com/api/hotel/list');
    let data = await response.json();
    let HotelData = data.result;
    const hotelsWithRoomType = HotelData.filter((result) => result.roomType.length > 0);
    return hotelsWithRoomType;
    // console.log("Test Data TEMPIK : ",JSON.stringify(hotelsWithRoomType));
  } catch (error) {
    console.error('Error:', error);
  }
}


const HotelData = [
  {
    id: '1',
    image: Images.room1,
    name: 'Boston Hotel',
    location: '553 Gislason Radial',
    price: '$125',
    available: 'Book 2 nights get free Breakfast',
    rate: 4.5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 100,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '2',
    image: Images.room2,
    name: 'Brisbane Hotel',
    location: '623 Cathrine Port',
    price: '$199',
    available: 'Book 2 nights get free Breakfast',
    rate: 4,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 99,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '3',
    image: Images.room3,
    name: 'Seattle Hotel',
    location: '303 Natasha Manors',
    price: '$399',
    available: 'Book 2 nights get free Breakfast',
    rate: 4,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 89,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '4',
    image: Images.room4,
    name: 'Chengdu Hotel',
    location: '19 Jennie Route',
    price: '$399',
    available: 'Only 4 rooms leaft',
    rate: 4.5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 299,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '5',
    image: Images.room5,
    name: 'Adora Hotel',
    location: '43 Gust Locks Apt. 725',
    price: '$499',
    available: 'Only 5 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 399,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '6',
    image: Images.room6,
    name: 'Cindy Hotel',
    location: '4873 Clotilde Isle',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '7',
    image: Images.room7,
    name: 'Tangle Hotel',
    location: '4873 Nevada St',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '8',
    image: Images.room8,
    name: 'Felix Hotel',
    location: '4873 Clotilde Isle',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '9',
    image: Images.room1,
    name: 'Happy Hotel',
    location: '553 Gislason Radial',
    price: '$125',
    available: 'Only 1 rooms leaft',
    rate: 4.5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 100,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '10',
    image: Images.room2,
    name: 'Lucky Hotel',
    location: '623 Cathrine Port',
    price: '$199',
    available: 'Only 2 rooms leaft',
    rate: 4,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 99,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '11',
    image: Images.room3,
    name: 'JoJo Hotel',
    location: '303 Natasha Manors',
    price: '$399',
    available: 'Only 3 rooms leaft',
    rate: 4,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 89,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '12',
    image: Images.room4,
    name: 'Sunflower Hotel',
    location: '19 Jennie Route',
    price: '$399',
    available: 'Only 4 rooms leaft',
    rate: 4.5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 299,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '13',
    image: Images.room5,
    name: 'Indianapolis Hotel',
    location: '43 Gust Locks Apt. 725',
    price: '$499',
    available: 'Only 5 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 399,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '14',
    image: Images.room6,
    name: 'San Jose Hotel',
    location: '4873 Clotilde Isle',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '15',
    image: Images.room7,
    name: 'Twin Peaks Hotel',
    location: '4873 Clotilde Isle',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
  {
    id: '16',
    image: Images.room8,
    name: 'Clarion Hotel',
    location: '4873 Clotilde Isle',
    price: '$299',
    available: 'Only 10 rooms leaft',
    rate: 5,
    rateStatus: 'Very Good',
    rateCount: '85 of 100',
    numReviews: 400,
    services: [
      {id: '1', icon: 'wifi', name: 'Free Wifi'},
      {id: '2', icon: 'coffee', name: 'Free Coffee'},
      {id: '3', icon: 'bath', name: 'Free Bath'},
      {id: '4', icon: 'car', name: 'Free Car'},
      {id: '5', icon: 'paw', name: 'Free Paw'},
      {id: '6', icon: 'futbol', name: 'Free Futbol'},
    ],
  },
];

export {HotelData};
